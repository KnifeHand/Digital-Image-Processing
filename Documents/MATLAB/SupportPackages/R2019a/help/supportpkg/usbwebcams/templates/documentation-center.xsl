...
$(function() {adjustTopNavWidth(false);});
    ...
    <div class="row">
      ...
      <div class="col-xs-12 col-sm-6">
        <div class="panel-group accordion accordion_variant_01" id="accordion_group_02" role="tablist" aria-multiselectable="true">
          ...
          <xsl:if test="$uniq_prod_categories[. = 'wireless-communications']">
            <xsl:call-template name="prod_category_accordion">
              <xsl:with-param name="prod_category" select="'wireless-communications'"></xsl:with-param>
            </xsl:call-template>
          </xsl:if>
  ...
  <xsl:template name="build_product_li" xml:id="HTML_2018_08_20_h11m00">
    ...
    <li>
      ...
      <a>
        <xsl:attribute name="href"><xsl:value-of select="concat(./help-location, '/index.html')"/></xsl:attribute>
  ...
  <xsl:template name="web_page_nextgen">
    ...
    <!-- Fix the path to search.html so that the search box in index-not-found pages work properly. See g1866938. -->
    <xsl:variable name="abs_path_to_searchHTML">
      <xsl:choose>
        <xsl:when test="$generate_index_not_found_page='yes'">
          <xsl:value-of select="concat('/help/releases/',$releaseversion,'/')" />
        </xsl:when>
        <xsl:otherwise/>
      </xsl:choose>
    </xsl:variable>
    ...
    <xsl:text disable-output-escaping="yes">&#x3C;@layoutComponents.doc_search action="</xsl:text><xsl:value-of select="$abs_path_to_searchHTML"/><xsl:text disable-output-escaping="yes">search.html"/&#x3E;&#x0A;</xsl:text>
