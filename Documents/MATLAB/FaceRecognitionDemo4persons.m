...
[r,c]=size(im); % Array returns the number of rows and columns when im is a matrix
num_im=length(imlist); % Length of largest array dimension
num_p=num_im/2;
x=zeros(r*c,num_p); % Create array of all zeros and define a single number of pixels
...
%% View four different weight combinations
%Every image of the same person has the same weights
%%%weight of the training data projected into eigen space%%%%%
wta=p2'*ata; % A*P2= P;  P'*A =Wt_A
figure,subplot(1,2,1),plot(wta(:,1));title('Weights of Person1');
subplot(1,2,2),plot(wta(:,2));title('Weights of Person2');
%subplot(1,4,3),plot(wta(:,3));title('Weights of Person3');
%subplot(1,4,4),plot(wta(:,4));title('Weights of Person4');
